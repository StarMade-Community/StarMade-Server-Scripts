...
# JVM args required by the pre branch (kept in sync with .env.example)
PRE_JVM_EXTRA_ARGS="-javaagent:StarMade.jar --add-opens=java.base/jdk.internal.misc=ALL-UNNAMED --add-opens=java.base/jdk.internal.ref=ALL-UNNAMED --add-opens=java.base/java.nio=ALL-UNNAMED"
...
if [ "$SETUP_METHOD" == "1" ]; then
    ...
    JAVA_VERSION=21
    JVM_EXTRA_ARGS=JVM_EXTRA_ARGS=-javaagent:StarMade.jar --add-opens=java.base/jdk.internal.misc=ALL-UNNAMED --add-opens=java.base/jdk.internal.ref=ALL-UNNAMED --add-opens=java.base/java.nio=ALL-UNNAMED
...
# The pre branch requires extra JVM args (see .env.example)
JVM_EXTRA_ARGS=""
if [ "$UPDATE_BRANCH" == "pre" ]; then
    JVM_EXTRA_ARGS="$PRE_JVM_EXTRA_ARGS"
fi
